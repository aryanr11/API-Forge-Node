...
exports.getParticularData = async function (req, res, next) {
    ...
    try {
        ...
        let nonNumericFields = Object.keys(PAYMENT.schema.paths).filter(field => field !== "_id" && (PAYMENT.schema.paths[field].instance !== 'Number'));
...
exports.update = async function (req, res, next) {
    ...
    try {
        let Data = await PAYMENT.findByIdAndUpdate(req.params.id, req.body,{new : true})
        if(!Data){
            throw new Error("Data Was Not Found")
        }
        res.status(201).json({
